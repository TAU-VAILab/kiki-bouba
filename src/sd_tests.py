...
def main():
    ...
    word2emb_fn = {w: f'{EMBEDDING_DIR}/{w}.pt' for w in words_kb}
    assert all(os.path.exists(fn) for fn in word2emb_fn.values()), "Missing embedding files"
    word2emb = {}
    with torch.no_grad():
        for word in tqdm(words_kb, desc="Loading embeddings"):
            fn = word2emb_fn[word]
            E = torch.load(fn).to(device) # (k, 1024)
            E = E.mean(dim=0)
            E /= E.norm()
            word2emb[word] = E
    ...
    def probe_s(word):
        E = word2emb[word]
        return probe(word, model, tokenizer, cats=sharp_cats, embedding=E)
    def probe_r(word):
        E = word2emb[word]
        return probe(word, model, tokenizer, cats=round_cats, embedding=E)
    ...
    sharp_E = torch.stack([
        word2emb[w]
        for w in sharp_words
    ])
    def score_adj(adj):
        return (
            probe(adj, model, tokenizer, cats=round_words,
                  add_shaped_word=False, add_shaped_cats=True, cat_embs=round_E)
            - probe(adj, model, tokenizer, cats=sharp_words,
                    add_shaped_word=False, add_shaped_cats=True, cat_embs=sharp_E)
        )
    ...
    SWE = torch.stack([word2emb[w] for w in sharp_words])
    RWE = torch.stack([word2emb[w] for w in round_words])
